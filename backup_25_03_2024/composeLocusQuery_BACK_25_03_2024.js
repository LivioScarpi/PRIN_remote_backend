function composeLocusQuery_BACK_25_03_2024(objectFilters, cameraPlacementLocusInRegionIDs, narrativeLocusInRegionIDs, cameraFilterRegionNotFilled, narrativeFilterRegionNotFilled) {
    ...
    if ((objectFilters.ucCastMemberName !== '' && objectFilters.ucCastMemberName !== null) || (objectFilters.ucCharacterName !== '' && objectFilters.ucCharacterName !== null) || (objectFilters.otherEntitiesType !== '' && objectFilters.otherEntitiesType !== null) || (cameraPlacementLocusInRegionIDs.length > 0 && !cameraFilterRegionNotFilled) || (narrativeLocusInRegionIDs.length > 0 && !narrativeFilterRegionNotFilled)) {
        ...
        const q = `SELECT r.id as resource_id FROM resource r join resource_class rc on r.resource_class_id=rc.id WHERE rc.local_name="PlaceRepresentationCatalogueRecord"`;
        query += q;
    }
...
function composeLocusRelationships(queries, ids, type, filter, locusRelationshipsDictionary) {
    ...
    */
    ...
    var q = `CREATE TEMPORARY TABLE IF NOT EXISTS ${filter}_locus_list_free_type AS (
            ...
            FROM (
                  SELECT resource_id
                  FROM table_join_free_type
                  WHERE FIND_IN_SET(resource_id, (
...
function composeLocusOverTime(queries, ids, type, filter, locusOverTimeRelationshipsDictionary) {
    ...
    CREATE TEMPORARY TABLE IF NOT EXISTS ${filter}_locus_over_time_list_free_type AS
        ...
        FROM table_join_locus_over_time_free_type
        WHERE FIND_IN_SET(t3_value_resource_id, (
            SELECT GROUP_CONCAT(ID SEPARATOR ',') AS All_List
            FROM (
...
*/
    ...
    var q = `
                ...
                SELECT t1_resource_id
        FROM table_join_locus_over_time_type_name
        WHERE FIND_IN_SET(t3_value_resource_id, (
            SELECT GROUP_CONCAT(ID SEPARATOR ',') AS All_List
            FROM (
                     SELECT ID FROM LocusOverTimeRelationships WHERE ID IN (${ids})
...
module.exports = {
    composeLocusQuery: composeLocusQuery_BACK_25_03_2024,
