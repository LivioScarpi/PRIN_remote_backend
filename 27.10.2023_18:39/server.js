...
function getFilmFilters(res, req) {
    ...
    //chiedo la lista di film
    var query = `SELECT distinct property_id, value FROM value where property_id in (844, 870, 868, 856, 857, 839, 938, 929, 1011, 932, 936, 931, 930, 934)`;
    ...
    filters.then(
        function ({list, res}) {
            ...
            var genres = list.filter(obj => obj.property_id === 844);
            var titleType = list.filter(obj => obj.property_id === 870);
            var titleLanguage = list.filter(obj => obj.property_id === 868);
            var productionCountry = list.filter(obj => obj.property_id === 856);
            var productionName = list.filter(obj => obj.property_id === 857);
            var dateTypology = list.filter(obj => obj.property_id === 839);

            /*linguaggio e stile*/
            var lighting = list.filter(obj => obj.property_id === 938);
            var cameraAngle = list.filter(obj => obj.property_id === 929);
            var tilt = list.filter(obj => obj.property_id === 1011);
            var cameraShotType = list.filter(obj => obj.property_id === 932);
            var matte = list.filter(obj => obj.property_id === 936);
            var pointOfView = list.filter(obj => obj.property_id === 931);
            var cameraMotion = list.filter(obj => obj.property_id === 930);
            var colouring = list.filter(obj => obj.property_id === 934);
